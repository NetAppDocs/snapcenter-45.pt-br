...
* A operação de backup iniciada por um usuário de diretório ativo (AD) falha se a credencial de instância SQL não for atribuída ao usuário ou grupo do AD. Você deve atribuir a credencial de instância SQL ao usuário ou grupo do AD a partir da página *Configurações* > *Acesso do usuário*.
