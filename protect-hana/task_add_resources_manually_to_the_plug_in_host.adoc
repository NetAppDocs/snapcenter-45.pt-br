...
Chaves de armazenamento de usuário seguro HDB
 a| 
Digite a chave para se conetar ao sistema SAP HANA. A chave contém as informações de login para se conetar ao banco de dados.
...
+
Os bancos de dados são exibidos juntamente com informações como SID, host do plugin, grupos e políticas de recursos associados e status geral
...
Não é possível modificar o seguinte se o recurso SAP HANA tiver backups associados a ele:
